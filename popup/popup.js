...
const renderTabList = (tabs, groups) => {
  ...
  // Separate tabs into pinned, grouped, and ungrouped
  const pinnedTabs = [];
  ...
  if (pinnedTabs.length > 0) {
    createPinnedHeader(pinnedTabs.length, popupList);
  ...
  if (ungroupedTabs.length > 0) {
    if (groupedTabs.size > 0 || pinnedTabs.length > 0) {
...
// Create a header for pinned tabs section
const createPinnedHeader = (tabCount, container) => {
  const markup = `
    <div class="tab-group-header" data-group-id="pinned" data-collapsed="false">
      <button class="group-collapse-btn" aria-expanded="true" aria-label="Toggle pinned tabs">
        <span class="collapse-icon">▼</span>
      </button>
      <input type="checkbox" class="group-checkbox" id="group-pinned" data-group-id="pinned">
      <label for="group-pinned" class="group-label">
